...
class DndgeneratorValidationException implements Exception {
  final List<String> errors;

  DndgeneratorValidationException(this.errors);

  @override
  String toString() => 'DndgeneratorValidationException: ${errors.join("; ")}';
}
...
/// D&D Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/dndgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [type] - Content type to generate
/// * [count] - Number of items to generate [min: 1, max: 10]
class DndgeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'type': ValidationRule(type: 'string', required: false),
    'count': ValidationRule(type: 'integer', required: false, min: 1, max: 10),
  };

  DndgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/dndgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DndgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DndgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the D&D Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<DndgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DndgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DndgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DndgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DndgeneratorException('Resource not found');
      } else {
        throw DndgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DndgeneratorException) rethrow;
      throw DndgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the D&D Generator API client.
class DndgeneratorException implements Exception {
  final String message;

  DndgeneratorException(this.message);

  @override
  String toString() => 'DndgeneratorException: $message';
}
